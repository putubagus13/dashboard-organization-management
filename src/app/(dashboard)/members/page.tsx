    ...
    <div className="space-y-6">
      ...
      <div className="bento-card p-5 space-y-4">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button onClick={fetchMembers} className="btn-secondary shrink-0">
            <RefreshCw size={14} />
          </button>
        </div>
          ...
          columns={[
            {
              ...
              header: 'No. Anggota',
              cell: (m) => (
                <span className="font-mono text-xs font-semibold text-slate-500">{m.member_number ?? '-'}</span>
              ),
            },
            {
              key: 'name',
            ...
            },
            {
              key: 'gender',
              header: 'L/P',
              cell: (m) => (m.gender === 'L' ? 'Laki-laki' : m.gender === 'P' ? 'Perempuan' : '-'),
            },
            {
              key: 'status',
              header: 'Status',
            ...
            },
            {
              key: 'role',
              header: 'Jabatan',
              cell: (m) => <span className="capitalize">{m.role.replace('_', ' ')}</span>,
            },
            { key: 'join', header: 'Bergabung', cell: (m) => formatDateShort(m.join_date) },
            {
              key: 'points',
              header: 'Poin',
              cell: (m) => <span className="font-semibold text-brand-700">{m.activity_points}</span>,
            },
            {
              key: 'active',
              header: 'Status',
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-slate-500">
            {count > 0
              ? `Menampilkan ${(page - 1) * PAGE_SIZE + 1}–${Math.min(page * PAGE_SIZE, count)} dari ${count}`
              : 'Tidak ada data'}
          </p>
      ...
      <Modal
        ...
        description={editing ? `Edit data ${editing.full_name}` : 'Isi data anggota baru'}
        size="lg"
      >
        <MemberForm
          member={editing}
