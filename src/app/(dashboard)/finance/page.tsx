  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Saldo" value={formatCurrency(totalBalance)}
          icon={Wallet} iconColor="text-brand-600" iconBg="bg-brand-50" />
        <StatCard title="Pemasukan Bulan Ini" value={formatCurrency(totalIncome)}
          icon={TrendingUp} iconColor="text-green-600" iconBg="bg-green-50" />
        <StatCard title="Pengeluaran Bulan Ini" value={formatCurrency(totalExpense)}
        ...
        <StatCard
          ...
          icon={Banknote} iconColor="text-amber-600" iconBg="bg-amber-50" />
      </div>
      ...
      <div className="bento-card p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {(accounts ?? []).map((acc) => {
            ...
            return (
              <div key={a.id} className="rounded-xl border border-slate-200 p-4 hover:border-brand-300 transition-colors">
        ...
        <div className="space-y-4">
          ...
          <div className="bento-card p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/finance/loans" className="text-sm text-brand-700 hover:underline">Semua</Link>
            </div>
              ...
              {loans.slice(0, 4).map((l) => (
                <div key={l.id} className="rounded-xl border border-slate-200 p-3">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Badge variant="info" size="sm">{l.loan_number}</Badge>
                  </div>
                  <p className="text-xs text-slate-400 mt-1">
                    Sisa: <span className="font-semibold text-red-600">{formatCurrency(l.remaining_amount ?? 0)}</span>
                  </p>
                  {l.due_date && (
                    <p className="text-xs text-slate-400">Jatuh tempo: {formatDateShort(l.due_date)}</p>
                  )}
                </div>
              ))}
              {loans.length === 0 && (
          ...
          <div className="bento-card p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <Link href="/finance/donors" className="text-sm text-brand-700 hover:underline">Semua</Link>
            </div>
            <div className="space-y-2">
              {donors.map((d, i) => (
                <div key={d.id} className="flex items-center gap-2">
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${i === 0 ? "bg-amber-400 text-white" : i === 1 ? "bg-slate-300 text-slate-700" : "bg-slate-100 text-slate-600"}`}>
                    {i + 1}
                  </span>
              ...
              {donors.length === 0 && (
                <p className="text-sm text-slate-400 text-center py-3">Belum ada donatur</p>
              )}
            </div>
          </div>
        </div>
