      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            <Link href="/dues/settings" className="btn-secondary">
              <Settings size={14} />
              Pengaturan Iuran
            </Link>
            <button
        ...
        <Table
          ...
          columns={[
              ...
              cell: (d) => (
                <div>
                  <p className="font-medium text-slate-900">
                    {(d.member as { full_name: string } | null)?.full_name ?? '-'}
                  </p>
                  <p className="text-xs text-slate-400">
                    {(d.member as { member_number: string } | null)?.member_number}
                  </p>
                </div>
              ),
            },
            ...
            { key: 'period', header: 'Periode', cell: (d) => formatPeriod(d.period_year, d.period_month) },
            {
              key: 'amount',
              header: 'Jumlah',
              cell: (d) => <span className="font-semibold">{formatCurrency(d.amount)}</span>,
            },
            {
              key: 'payment_date',
              header: 'Tgl Bayar',
              cell: (d) => (d.status === 'paid' ? formatDate(d.payment_date) : '-'),
            },
            {
              key: 'status',
              header: 'Status',
              cell: (d) => (
                <span
                  className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${getDuesStatusColor(d.status)}`}
                >
            ...
            },
            {
              key: 'notes',
              header: 'Catatan',
              cell: (d) => <span className="text-slate-400 text-xs">{d.notes ?? '-'}</span>,
            },
            {
              key: 'actions',
              header: '',
