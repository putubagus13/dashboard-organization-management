...
export default function WasteLeaderboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (period === "all") {
      ...
      (data ?? []).forEach((d) => {
        const c = (d.collector as unknown) as { id: string; name: string; rt_rw: string | null; is_member: boolean } | null;
        if (!c) return;
        if (!agg[d.collector_id]) {
          agg[d.collector_id] = { id: c.id, name: c.name, rt_rw: c.rt_rw, is_member: c.is_member, member: null, weight: 0, earnings: 0 };
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-1 p-1 bg-slate-100 rounded-xl w-fit">
        {(["all", "month", "year"] as Period[]).map((p) => (
          <button
            key={p}
            onClick={() => setPeriod(p)}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${period === p ? "bg-white shadow-sm text-slate-900" : "text-slate-500 hover:text-slate-700"}`}>
      ...
      <div className="grid grid-cols-3 gap-4">
        <StatCard title="Total Berat" value={formatWeight(totals.weight)} icon={Scale} iconColor="text-brand-600" iconBg="bg-brand-50" />
        <StatCard title="Total Nilai" value={formatCurrency(totals.earnings)} icon={Banknote} iconColor="text-green-600" iconBg="bg-green-50" />
        <StatCard title="Total Pengepul" value={totals.collectors} icon={Users} iconColor="text-blue-600" iconBg="bg-blue-50" />
      </div>
          ...
          <div className="flex items-end justify-center gap-6 max-w-lg mx-auto">
            ...
            {podium[1] && (
              <div className="flex flex-col items-center gap-2 flex-1">
                ...
                <p className="text-sm font-semibold text-slate-800 text-center leading-tight max-w-full truncate">{podium[1].name}</p>
            ...
            <div className="flex flex-col items-center gap-2 flex-1">
              ...
              <p className="text-sm font-bold text-slate-900 text-center leading-tight max-w-full truncate">{podium[0].name}</p>
            ...
            {podium[2] && (
              <div className="flex flex-col items-center gap-2 flex-1">
                ...
                <p className="text-sm font-semibold text-slate-800 text-center leading-tight max-w-full truncate">{podium[2].name}</p>
            ...
            {collectors.map((c, i) => (
              <div
                ...
                className={`flex items-center gap-4 p-4 rounded-xl border transition-all
                  ${i === 0 ? "border-amber-300 bg-amber-50" : i === 1 ? "border-slate-300 bg-slate-50/60" : i === 2 ? "border-amber-800/20 bg-orange-50/30" : "border-slate-200 bg-white hover:bg-slate-50"}`}>
                <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0 shadow-sm ${medalStyle(i)}`}>
                  {i + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-semibold text-slate-900">{c.name}</p>
                    {c.is_member && <Badge variant="info" size="sm">Anggota</Badge>}
                    {c.rt_rw && <span className="text-xs text-slate-400">RT/RW {c.rt_rw}</span>}
                  </div>
                  {c.member && (
                    <p className="text-xs text-slate-400 mt-0.5">
                      {(c.member as { full_name: string }).full_name}
                    </p>
                  )}
                </div>
