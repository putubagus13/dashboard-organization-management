  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <button onClick={() => { setEditingSession(undefined); setShowSessionModal(true); }} className="btn-primary">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <StatCard title="Selesai" value={completed} icon={CheckCircle} iconColor="text-green-600" iconBg="bg-green-50" />
      </div>

      <div className="bento-card p-5 space-y-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <SearchInput value={search} onChange={v => { setSearch(v); setPage(1); }} placeholder="Cari sesi..." className="flex-1" />
          <select value={filterStatus} onChange={e => { setFilterStatus(e.target.value); setPage(1); }} className="input-base w-full sm:w-40">
        ...
        <Table
          ...
          columns={[
            { key: "title", header: "Judul Sesi", cell: s => (
              <div>
            ...
            { key: "weight", header: "Total Berat", cell: s => (
              <span className="font-semibold text-slate-800">{formatWeight(s.total_weight)}</span>
            )},
            { key: "earnings", header: "Total Nilai", cell: s => (
              <span className="font-semibold text-brand-700">{formatCurrency(s.total_earnings)}</span>
            )},
            ...
            { key: "actions", header: "", cell: s => (
              <div className="flex gap-1">
                <button onClick={() => { setActiveSession(s); setShowDetailModal(true); }}
                  className="p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50" title="Detail">
                  <Eye size={14} />
                </button>
                {s.status === "ongoing" && (
                  <>
                    <button onClick={() => { setActiveSession(s); setShowCollectModal(true); }}
                      className="p-1.5 rounded-lg text-slate-400 hover:text-brand-600 hover:bg-brand-50" title="Catat Sampah">
                      <Plus size={14} />
                    </button>
                    <button onClick={() => handleCompleteSession(s)}
                      className="p-1.5 rounded-lg text-slate-400 hover:text-green-600 hover:bg-green-50" title="Selesaikan">
                      <CheckCircle size={14} />
                    </button>
                  </>
                )}
      ...
      {/* Session Form Modal */}
      <Modal open={showSessionModal} onClose={() => setShowSessionModal(false)}
        title={editingSession ? "Edit Sesi" : "Buat Sesi Pengumpulan"} size="md">
        <WasteSessionForm
          session={editingSession}
          onSuccess={() => { setShowSessionModal(false); fetchData(); }}
          onCancel={() => setShowSessionModal(false)} />
      </Modal>

      {/* Collection Form Modal */}
      <Modal open={showCollectModal} onClose={() => setShowCollectModal(false)}
        title="Catat Pengumpulan Sampah" size="lg">
        <WasteCollectionForm
          session={activeSession}
          onSuccess={() => { setShowCollectModal(false); fetchData(); }}
          onCancel={() => setShowCollectModal(false)} />
      </Modal>

      {/* Detail Modal */}
      <Modal open={showDetailModal} onClose={() => setShowDetailModal(false)}
        title={`Detail Sesi: ${activeSession?.title}`} size="xl">
        <WasteCollectionDetail
          session={activeSession}
          onAddCollection={() => { setShowDetailModal(false); setShowCollectModal(true); }} />
      </Modal>

      <ConfirmDialog
        open={!!deleteTarget} title="Hapus Sesi"
