...
import { withUpdateAudit } from '@/lib/audit';
...
export default function LoanForm({ loan, onSuccess, onCancel }: Props) {
  ...
  function up<K extends keyof LoanFormData>(k: K, v: LoanFormData[K]) {
    setForm((f) => ({ ...f, [k]: v }));
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    const payload = {
      p_borrower_name: form.borrower_name,
      p_member_id: form.member_id || null,
      p_account_id: form.account_id,
      p_principal_amount: form.principal_amount,
      p_interest_rate: form.interest_rate ?? 0,
      p_loan_date: form.loan_date,
      p_due_date: form.due_date || null,
      p_purpose: form.purpose || null,
      p_collateral: form.collateral || null,
    ...
    const { error: err } = loan
      ? // ? await supabase.rpc('update_loan_with_transaction', { p_loan_id: loan.id, ...payload })
        await supabase
          .from('loans')
          .update(await withUpdateAudit(supabase, payloadEdit))
          .eq('id', loan.id)
      : await supabase.rpc('create_loan_with_transaction', payload);
    if (err) {
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2">
          ...
          <input
            ...
            onChange={(e) => up('borrower_name', e.target.value)}
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => up('account_id', e.target.value)}
            className="input-base"
            required
          >
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => up('principal_amount', Number(e.target.value))}
            className="input-base"
            required
            disabled={!!loan}
          />
        ...
        <div>
          ...
          <input
            ...
            min="0"
            step="0.1"
            value={form.interest_rate ?? 0}
            onChange={(e) => up('interest_rate', Number(e.target.value))}
            className="input-base"
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) => up('loan_date', e.target.value)}
            className="input-base"
            required
          />
        </div>
        <div>
          ...
          <input
            type="date"
            value={form.due_date ?? ''}
            onChange={(e) => up('due_date', e.target.value || undefined)}
            className="input-base"
          />
        </div>
        <div className="col-span-2">
          ...
          <input
            type="text"
            value={form.purpose ?? ''}
            onChange={(e) => up('purpose', e.target.value || undefined)}
            className="input-base"
          />
        </div>
        <div className="col-span-2">
          ...
          <input
            ...
            onChange={(e) => up('collateral', e.target.value || undefined)}
            className="input-base"
          />
        </div>
        <div className="col-span-2">
          ...
          <textarea
            rows={2}
            value={form.notes ?? ''}
            onChange={(e) => up('notes', e.target.value || undefined)}
