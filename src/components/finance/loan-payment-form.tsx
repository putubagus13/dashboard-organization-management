...
export default function LoanPaymentForm({ loan, onSuccess, onCancel }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError('');
    const { error: err } = await supabase.rpc('create_loan_payment_with_transaction', {
      p_loan_id: loan.id,
      p_amount: amount,
      p_payment_date: date,
      p_notes: notes || null,
  ...
  const maxLoanPayment = loan.remaining_amount ?? 0;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="rounded-xl bg-slate-50 p-4 space-y-1">
        ...
        <p className="text-sm">
          Sisa: <span className="font-bold text-red-600">{formatCurrency(loan.remaining_amount ?? 0)}</span>
        </p>
      </div>
      <div>
        ...
        <input
          type="number"
          min="1"
          max={loan.remaining_amount ?? undefined}
          value={amount}
