    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          ...
          <input type="text" value={form.full_name} onChange={e => update("full_name", e.target.value)} className="input-base" required />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="date" value={form.date_of_birth ?? ""} onChange={e => update("date_of_birth", e.target.value)} className="input-base" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1.5">Jenis Kelamin</label>
          <select value={form.gender ?? ""} onChange={e => update("gender", e.target.value as "L" | "P" | undefined || undefined)} className="input-base">
        ...
        <div>
          ...
          <input type="text" value={form.occupation ?? ""} onChange={e => update("occupation", e.target.value)} className="input-base" />
        </div>
        <div>
        ...
        <div>
          ...
          <input type="date" value={form.join_date ?? ""} onChange={e => update("join_date", e.target.value)} className="input-base" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1.5">Status Anggota</label>
          <select value={form.status_id ?? ""} onChange={e => update("status_id", e.target.value || undefined)} className="input-base">
        ...
        <div>
          ...
          <select value={form.role ?? "anggota"} onChange={e => update("role", e.target.value as MemberFormData["role"])} className="input-base">
        ...
        <div className="sm:col-span-2">
          ...
          <textarea rows={2} value={form.notes ?? ""} onChange={e => update("notes", e.target.value)} className="input-base resize-none" />
        </div>
        <div className="flex items-center gap-2">
          <input type="checkbox" id="is_active" checked={form.is_active} onChange={e => update("is_active", e.target.checked)} className="w-4 h-4 accent-brand-600 rounded" />
          <label htmlFor="is_active" className="text-sm font-medium text-slate-700">Anggota Aktif</label>
        </div>
      </div>
